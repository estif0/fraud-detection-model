...
class TestEnsembleModel:
    ...
    def test_predict_with_threshold(self, sample_data):
        ...
        pred_default = model.predict_with_threshold(X, threshold=0.5)
        assert len(pred_default) == len(X)
        assert set(pred_default).issubset({0, 1})

        # Test with high threshold (more conservative)
        pred_high = model.predict_with_threshold(X, threshold=0.9)
        assert len(pred_high) == len(X)
        # High threshold should predict fewer positives
        assert pred_high.sum() <= pred_default.sum()

        # Test with low threshold (more aggressive)
        pred_low = model.predict_with_threshold(X, threshold=0.1)
...
class TestCrossValidator:
    ...
    def test_cross_validate_model(self, sample_data):
        ...
        scoring_functions = {"f1": f1_score, "precision": precision_score}
